$(function(){
  ...
  function initInputsPlaceholders() {
    var placeholderText = '';
  ...
  function initInputsCleaner() {
    var inputElem = $('.main-filter__control--with-cleaner');
    var cleanButton = $('.main-filter__control-cleaner');

    if(inputElem.length === 0) return false;

    inputElem.find('.main-filter__control-cleaner').addClass('hidden');

    inputElem.find('input').on('input', function (e) {
      if(e.target.value === ''){
        hideCleanButton($(this));
      }else {
        showCleanButton($(this));
      }
    });
    ...
    cleanButton.on('click', function(e){
      ...
      $(this).closest('.main-filter__control--with-cleaner').find('input').val('');
      hideCleanButton($(this));
    });

    function showCleanButton(_this){
      _this.closest('.main-filter__control--with-cleaner').find('.main-filter__control-cleaner').removeClass('hidden');
    }
    function hideCleanButton(_this){
      _this.closest('.main-filter__control--with-cleaner').find('.main-filter__control-cleaner').addClass('hidden');
    }
  }
  function initAutocomplete() {
    var placeTypeInput = $('#clientsControl');

    if(placeTypeInput.length){
      placeTypeInput.easyAutocomplete({
        data: [
